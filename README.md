...
- `cmd/phase_3/` - RAG 示例
  - `1_simple_rag/` - 基于 SQLite FTS5 的最小本地 RAG 示例
...
Phase 3 当前包含一个基于 SQLite FTS5 的简单 RAG 示例，详细说明见 `cmd/phase_3/1_simple_rag/README.md`。
...
- SQLite 驱动：`github.com/glebarez/sqlite`
...
- `cmd/phase_3/1_simple_rag`：演示如何把本地文档写入 SQLite FTS5，再通过 `search_fts5` 工具接入 LLM 多轮问答流程。
- `pkg/rag/fts5`：封装 FTS5 表初始化、文档写入、删除和全文搜索，适合本地 demo 或轻量知识库检索。
- `search_fts5` 现已支持空格分隔的多个关键词，例如 `猫鼠队 上大分`，底层会转换为 `AND` 查询。
...
- `cmd/phase_3/1_simple_rag/README.md` - Simple RAG 示例说明
...
5. Phase 3 / RAG：阅读 `cmd/phase_3/1_simple_rag/README.md` 与 `pkg/rag/fts5/README.md`，理解本地知识库、全文检索和工具调用如何串成一个最小 RAG 闭环。
